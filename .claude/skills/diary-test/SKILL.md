...
name: diary-test
description: |-
  프로젝트 내 클래스/Composable에 대한 테스트 코드(TC) 작성 요청 시 실행한다.
  TRIGGER when: "테스트 작성해줘", "TC 추가", "테스트 만들어줘", "테스트 코드 짜줘", "이거 테스트해줘" 같은 테스트 파일 생성 요청; 특정 클래스/Composable에 대한 테스트 커버리지 추가 요청.
  SKIP: 기존 테스트 실행만 요청("테스트 돌려줘"); 테스트 실패 디버깅; 테스트 전략/방법론 일반 질문; "테스트 있어?" 같은 현황 조회.
...
- `core/mapper/CLAUDE.md`의 규칙(함수 순서, 테스트 이름, TC 순서)을 따른다.
- **커버리지 100%를 목표로 한다.** 모든 mapper 함수는 예외 없이 TC를 작성한다.
