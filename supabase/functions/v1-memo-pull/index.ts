...
import { MemoPullRequestV1 } from "./entity/memo-pull-request-v1.ts";
import { MemoV1 } from "../v1-memo-push/entity/memo-v1.ts";

const memoRepository = new MemoRepository(supabaseAdmin);

function toMemoV1(memo: Memo): MemoV1 {
  return {
    id: memo.id,
    detail: {
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { updatedAt }: MemoPullRequestV1 = await req.json();
    const memoList = await memoRepository.getUpdatedAfter(user.id, updatedAt);

    return new Response(JSON.stringify(memoList.map(toMemoV1)), {
      status: 200,
      headers: { "Content-Type": "application/json" },
    });
  } catch (error) {
