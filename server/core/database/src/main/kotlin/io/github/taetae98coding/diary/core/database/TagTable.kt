...
public data object TagTable : IdTable<String>(name = "Tag") {
    ...
    public fun findByUpdateAt(uid: String, updateAt: Instant): List<Tag> {
        return join(otherTable = TagAccountTable, joinType = JoinType.INNER) { ID eq TagAccountTable.TAG_ID }
            .selectAll()
            .where { (TagAccountTable.OWNER eq uid) and (UPDATE_AT greater updateAt) }
            .orderBy(UPDATE_AT)
            .limit(50)
            .map { it.toTag() }
    }
