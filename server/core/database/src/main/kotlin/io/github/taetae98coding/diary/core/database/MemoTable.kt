...
public data object MemoTable : IdTable<String>(name = "Memo") {
	...
	public fun findById(id: String): Memo? = selectAll()
		.where { ID eq id }
		.singleOrNull()
		?.toMemo()
	...
	public fun findByUpdateAtOwner(owner: String, updateAt: Instant): List<Memo> = join(otherTable = MemoAccountTable, joinType = JoinType.INNER) { ID eq MemoAccountTable.MEMO_ID }
		.selectAll()
	...
	public fun findGroupMemoByDateRange(groupId: String, dateRange: ClosedRange<LocalDate>): List<Memo> = join(otherTable = MemoBuddyGroupTable, joinType = JoinType.INNER) { ID eq MemoBuddyGroupTable.MEMO_ID }
		.selectAll()
		.where {
