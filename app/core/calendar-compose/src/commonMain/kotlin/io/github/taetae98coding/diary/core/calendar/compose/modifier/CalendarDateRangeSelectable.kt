...
public fun Modifier.calendarDateRangeSelectable(
    ...
    return pointerInput(state) {
        ...
        fun getSelectDate(): ClosedRange<LocalDate> {
            return minOf(dragStartDate, currentDragDate)..maxOf(dragStartDate, currentDragDate)
        }
...
private fun PointerInputScope.offsetToRowColumn(offset: Offset): Pair<Float, Float> {
    return (offset.y * 6 / size.height) to (offset.x * 7 / size.width)
}

private fun CalendarState.localDate(row: Float, column: Float): LocalDate {
    return LocalDate(year, month, 0, DayOfWeek.SUNDAY)
