...
internal class AppState(
    ...
    val topLevelNavKeys = TopLevelDestination.entries.map(TopLevelDestination::navKey).toSet()

    val currentTopLevelDestination by derivedStateOf {
        val topLevelNavKey = backStack.lastOrNull { it in topLevelNavKeys }
        TopLevelDestination.entries.find { it.navKey == topLevelNavKey }
    }

    val isNavigationVisible by derivedStateOf {
        backStack.lastOrNull() in topLevelNavKeys
    }
