...
class FinishMemoUseCaseTest : BehaviorSpec() {
    ...
    private val useCase = FinishMemoUseCase(requestSyncUseCase, accountMemoRepository)
    ...
    init {
        Given("유효한 memoId") {
            ...
            coEvery { accountMemoRepository.updateFinish(memoId = memoId, isFinished = true) } returns Unit

            When("FinishMemoUseCase를 호출하면") {
                ...
                Then("repository의 updateFinish를 isFinished=true로 호출한다") {
                    coVerify(exactly = 1) { accountMemoRepository.updateFinish(memoId = memoId, isFinished = true) }
                }
