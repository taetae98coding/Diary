...
class RestartRoutineUseCaseTest : BehaviorSpec() {
    ...
    private val useCase = RestartRoutineUseCase(requestSyncUseCase, accountRoutineRepository)
    ...
    init {
        Given("유효한 routineId") {
            ...
            coEvery { accountRoutineRepository.updateFinish(routineId = routineId, isFinished = false) } returns Unit

            When("RestartRoutineUseCase를 호출하면") {
                ...
                Then("repository의 updateFinish를 isFinished=false로 호출한다") {
                    coVerify(exactly = 1) { accountRoutineRepository.updateFinish(routineId = routineId, isFinished = false) }
                }
